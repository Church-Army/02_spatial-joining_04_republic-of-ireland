...
sa <- read_sf(here("geospatial-data", "civic-areas",
                   "2015_sa-boundaries_republic-of-ireland"))
```
...
```{r tidyOutputAreas}
sa <- select(sa, sa_2015_code = SMALL_AREA)
head(sa)
```
...
overlap[[1]] |>
  ...
  select(sa_2015_code, sa_area, overlapping_area) |>
...
a7_sa_codes <- overlap[["ballina"]][["sa_2015_code"]]
a7_sa <- filter(sa, sa_2015_code %in% a7_sa_codes)
...
overlap_stats <- map(overlap_stats, select,
                     sa_2015_code, ratio_sa_intersection_com)
